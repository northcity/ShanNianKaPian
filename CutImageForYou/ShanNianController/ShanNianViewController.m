...
@interface ShanNianViewController ()<IFlySpeechRecognizerDelegate,IFlyPcmRecorderDelegate,IFlyRecognizerViewDelegate,PcmPlayerDelegate,AVAudioPlayerDelegate,WKNavigationDelegate,WKUIDelegate>
...
- (void)createBaseUI{
    ...
    [self.beginSpeakButton addTarget:self action:@selector(startBtnHandler:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)startBtnHandler:(UIButton *)sender {

    NSLog(@"%s[IN]",__func__);
    [self.volumArray removeAllObjects];
...
-(void)initRecognizer
{
    ...
    if ([IATConfig sharedInstance].haveView == NO) {
        ...
        [_pcmRecorder setSaveAudioPath:@"NEWLUYINWENJIAN"];    //not save the audio file
...
- (void) onResults:(NSArray *) results isLast:(BOOL)isLast{
    ...
    _result =[NSString stringWithFormat:@"%@%@", _speakLabel.text,resultString];
...
- (void) onIFlyRecorderBuffer: (const void *)buffer bufferSize:(int)size
{
    NSData *audioBuffer = [NSData dataWithBytes:buffer length:size];
            ...
            [audioBuffer writeToFile:[self filePathWithName:@"tmp.mp3"] atomically:YES];
         NSLog(@"data length:%d", [audioBuffer length]);
